...
const INITIAL_CMARS_AMOUNT = utils.parseEther('50000');
const INITIAL_SMARS_AMOUNT = utils.parseEther('10000');
const INITIAL_BMARS_AMOUNT = utils.parseEther('5');
  ...
  describe('seigniorage', () => {
    describe('#allocateSeigniorage', () => {
      beforeEach('transfer permissions', async () => {
        await bond.mint(operator.address, INITIAL_BMARS_AMOUNT);
        await cash.mint(operator.address, INITIAL_CMARS_AMOUNT);
        await cash.mint(treasury.address, INITIAL_CMARS_AMOUNT);
        await share.mint(operator.address, INITIAL_SMARS_AMOUNT);
    ...
    describe('after startTime', () => {
      ...
      describe('#redeemBonds', () => {
        beforeEach('allocate seigniorage to treasury', async () => {
          await cash.mint(operator.address, INITIAL_CMARS_AMOUNT);
          await bond.mint(operator.address, INITIAL_CMARS_AMOUNT);
          for await (const contract of [cash, bond, share, boardroom]) {
            await contract.connect(operator).transferOperator(treasury.address);
          }
