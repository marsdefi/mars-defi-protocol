...
import {IMarsAsset} from '../assets/IMarsAsset.sol';
...
contract Treasury is ContractGuard, Epoch {
    ...
    modifier checkOperator {
        require(
            IMarsAsset(cash).operator() == address(this) &&
                IMarsAsset(bond).operator() == address(this) &&
                IMarsAsset(share).operator() == address(this) &&
        ...
        updatePrice
    {
        ...
        IMarsAsset(cash).burnFrom(_msgSender(), amount);
        IMarsAsset(bond).mint(_msgSender(), amount.mul(1e18).div(cashPrice));
        ...
        updatePrice
    {
        ...
        IMarsAsset(bond).burnFrom(_msgSender(), amount);
        ...
        checkOperator
    {
        ...
        IMarsAsset(cash).mint(address(this), seigniorage);
